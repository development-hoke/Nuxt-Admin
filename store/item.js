...
export const actions = {
  async fetch({ commit }, params = {}) {
    const { data, meta } = await api.item.fetch(params)
  ...
  async fetchOne({ commit }, { id }) {
    const { data } = await api.item.fetchOne(id)

    commit('itemsUsedSameStyling/set', data.itemsUsedSameStylings, {
      root: true,
    })
    commit('recommendItem/set', data.recommendItems, {
      root: true,
    })

    delete data.itemsUsedSameStylings
    delete data.recommendItems

    commit('setOne', data)
  },

  async update({ commit }, { id, params }) {
    const { data } = await api.item.update(id, params)
  ...
  async updateStatus({ commit }, { id, status }) {
    const { data } = await api.item.updateStatus(id, status)
  ...
  async nextPage({ commit, state }, payload) {
    ...
    const { data } = await api.item.fetch({
      ...params,
