...
export const actions = {
  async fetch({ commit }, params = {}) {
    const { data, meta } = await api.event.fetch(params)
  ...
  async fetchOne({ commit }, { id }) {
    const { data } = await api.event.fetchOne(id)

    commit('setOne', data)
  },

  async create({ commit }, { params }) {
    const { data } = await api.event.create(params)

    commit('setOne', data)
  },

  async update({ commit }, { id, params }) {
    const { data } = await api.event.update(id, params)
