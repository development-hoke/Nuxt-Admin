...
export const actions = {
  async fetch({ commit }, params = {}) {
    const { data, meta } = await api.help.fetch(params)
  ...
  async fetchOne({ commit }, { id }) {
    const { data } = await api.help.fetchOne(id)

    commit('setOne', data)
  },

  async create({ commit }, { params }) {
    const { data } = await api.help.create(params)

    commit('setOne', data)
  },

  async update({ commit }, { id, params }) {
    const { data } = await api.help.update(id, params)
