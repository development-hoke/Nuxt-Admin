...
export const getters = {
  ...
  options: ({ data }) => createOnlineCategoryOptions(data),
}
...
export const actions = {
  async fetch({ commit }, params = {}) {
    const { data } = await api.onlineCategory.fetch(params)

    commit('set', data)
  },

  async create({ commit }, { params }) {
    const { data } = await api.onlineCategory.create(params)

    commit('set', data)
  },

  async update({ commit }, { id, params }) {
    const { data } = await api.onlineCategory.update(id, params)
  ...
  async deleteNode({ commit, getters }, { id }) {
    ...
    await api.onlineCategory.delete(id)
