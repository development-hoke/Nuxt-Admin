import api from '@/api'
import { createTree, createTreeCache } from '@/utils/dataStructure'
...
export const actions = {
  async fetch({ commit }, params = {}) {
    const { data } = await api.helpCategory.fetch(params)

    commit('set', data)
  },

  async create({ commit }, { params }) {
    const { data } = await api.helpCategory.create(params)

    commit('set', data)
  },

  async update({ commit }, { id, params }) {
    const { data } = await api.helpCategory.update(id, params)
  ...
  async deleteNode({ commit, getters }, { id }) {
    ...
    await api.helpCategory.delete(id)
