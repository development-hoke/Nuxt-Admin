...
const MAX_SETTING_COUNT = 4

export default {
  name: 'TopContentNewItemAddModal',
  emits: ['opened', 'closed'],
  components: {
    ItemSortAddTable,
  ...
  methods: {
    ...
    handleAdd() {
      ...
      if (!this.isAcceptableCount(ids)) {
        this.error = `件数が多すぎます。${MAX_SETTING_COUNT}件までしか登録できません。`
        return
      }
    ...
    isAcceptableCount(ids) {
      const currentItems = this.currentItems
      return ids.length + currentItems <= MAX_SETTING_COUNT
    },
  },
}
</script>
