...
export default {
  ...
  computed: {
    ...
    title() {
      return this.isCreateMode ? 'ヘルプカテゴリを追加' : 'ヘルプカテゴリを編集'
    },
  },
  beforeDestroy() {
    ...
    async handleClickSave() {
      ...
      try {
        ...
        if (this.mode === MODE_CREATE) {
          await this.$store.dispatch('helpCategory/create', {
            params,
          })
          this.$toast.success('ヘルプカテゴリを追加しました。')
        } else {
        ...
        this.$toast.error('ヘルプカテゴリの保存に失敗しました。')
        console.error(error)
        this.serverErrors = {}
