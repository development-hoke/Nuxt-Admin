...
export default {
  ...
  computed: {
    ...
    title() {
      return this.isCreateMode ? 'タグを追加' : 'タグを編集'
    },
    ...mapGetters({
      tree: 'onlineTag/tree',
    }),
    numberRules() {
      const tagId = this.onlineTag ? this.onlineTag.id : null
    ...
    async handleClickSave() {
      ...
      try {
        ...
        if (this.mode === MODE_CREATE) {
          await this.$store.dispatch('onlineTag/create', {
            params,
          })
          this.$toast.success('オンラインタグを追加しました。')
        } else {
